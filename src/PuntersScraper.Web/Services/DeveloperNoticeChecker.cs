...
/// <summary>A developer announcement to show on the next page load, published via notice.json at
/// the repo root (see send-notice.ps1) — same schema the desktop App polls. An empty
/// <see cref="Id"/> means "nothing to show".</summary>
...
/// cutting a full release for a one-off message: the developer edits one file and pushes, no
/// rebuild/redeploy needed. The Web app shows it until an admin explicitly dismisses it (see
/// WebAppSettings.LastSeenNoticeId, shared across every user of this deployment); publishing a
/// new note just means giving it a new Id, which makes it show again even after an earlier one
/// was dismissed.
/// </summary>
public static class DeveloperNoticeChecker
{
    private const string RawUrl =
        "https://raw.githubusercontent.com/Vidusha-Mindula/PuntersWeb-Scraper/main/notice.json";

    /// <summary>Returns the current notice, or null if there isn't one / the check failed for any
    /// reason (offline, file missing, malformed) — this must never block startup or bother an
    /// admin with a failed background check.</summary>
