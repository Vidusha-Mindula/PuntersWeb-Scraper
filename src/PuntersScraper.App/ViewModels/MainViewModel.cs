...
public sealed partial class MainViewModel : ObservableObject
{
    ...
    // last decides the saved value instead of the one the user actually clicked.
    partial void OnUseChromeBrowserChanged(bool value)
    {
        if (_loadingSettings || !value) return;
        _settings.ScraperBrowser = nameof(ScraperBrowserChoice.Chrome);
        _settings.Save();
    }
    ...
    {
        if (_loadingSettings || !value) return;
        _settings.ScraperBrowser = nameof(ScraperBrowserChoice.Firefox);
        _settings.Save();
    }
    ...
    {
        if (_loadingSettings || !value) return;
        _settings.ScraperBrowser = nameof(ScraperBrowserChoice.Edge);
        _settings.Save();
    }
    ...
    {
        if (_loadingSettings) return;
        _settings.AutoScrapeEnabled = value;
        _settings.Save();
    }
