...
public static class UpdateChecker
{
    ...
    /// (e.g. "3.14.0") — the single place MainWindow's title bar reads this from, so it can never
    ...
    /// LIST endpoint (up to 30 most recent) rather than "/releases/latest" for that reason. Web
    /// releases share this repo but are tagged "web-vX.Y.Z" and marked prerelease (see
    /// UpdateAvailabilityChecker) — not installable here, so they're skipped. Returns an empty
    /// list rather than throwing on any failure (offline, rate-limited, repo not configured) —
    /// same "never bother the user with a plumbing error" rule as <see cref="CheckAsync"/>.</summary>
    public static async Task<List<ReleaseInfo>> ListReleasesAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            foreach (var release in doc.RootElement.EnumerateArray())
            {
                ...
                if (tag is null || tag.StartsWith("web-v", StringComparison.OrdinalIgnoreCase)) continue;
